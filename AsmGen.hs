...
data AsmFunc = AsmFunc String [Instruction] deriving (Show)
...
data Instruction = Add  Reg Reg Reg
...
progToAsm :: Program -> AsmProg
progToAsm (Program f) = AsmProg $ funcToAsm f

funcToAsm :: Function -> AsmFunc
funcToAsm (Function name body) = AsmFunc name (stmtToAsm body)

stmtToAsm :: Statement -> [Instruction]
stmtToAsm (Return expr) = exprToAsm expr ++ [Sys Exit]

exprToAsm :: Expr -> [Instruction]
exprToAsm (Lit n) = [Addi R7 R0 n]
